# Standby
A modular, customizable and open source app similar to the StandBy mode on iOS devices for Android.  

Plugins are fully user customizable. See example in `app/src/assets/examples/sample_plugin` and `SPEC.md` for specific plugin formats.  

Supports full screen plugins as well as two plugins side by side.  

(THIS PROJECT IS VERY MUCH IN WIP!)  